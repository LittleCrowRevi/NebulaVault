...
pub fn generate_room(min_size: i32, max_size: i32, x_range: i32, y_range: i32) -> Rect {
    let mut rng = thread_rng();
    let w = rng.gen_range(min_size..max_size);
    let h = rng.gen_range(min_size..max_size);
    let x = rng.gen_range(1..x_range - w - 1);
    let y = rng.gen_range(1..y_range - h - 1);
    Rect::new(x, y, w, h)
}
...
pub fn new_map_rooms_coors() -> (Vec<TileType>, Vec<Rect>) {
    ...
    for _ in 0..MAX_ROOMS {
        let n_room = generate_room(MIN_SIZE, MAX_SIZE, DEVMAP_SIZE.x(), DEVMAP_SIZE.y());

        if rooms.iter().any(|r| n_room.intersect(r)) {
            continue;
        }

        apply_room_to_map(&mut map, &n_room);

        if !rooms.is_empty() {
            let (nx, ny) = n_room.center();
            let (px, py) = rooms[rooms.len() - 1].center();
            if rng.gen_bool(0.5) {
                apply_htunnel(&mut map, px, nx, py, 1);
                apply_vtunnel(&mut map, py, ny, nx, 1);
            } else {
                apply_vtunnel(&mut map, py, ny, px, 1);
                apply_htunnel(&mut map, px, nx, ny, 1);
            }
        }
        rooms.push(n_room);
    }
