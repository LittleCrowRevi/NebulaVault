...
impl Rect {
    pub fn new(x: i32, y: i32, w: i32, h: i32) -> Rect { Rect { x1: x, y1: y, x2: x + w, y2: y + h } }

    /// Check if the given Rect intersects with this Rect.
    pub fn intersect(&self, other: &Rect) -> bool { self.x1 <= other.x2 && self.x2 >= other.x1 && self.y1 <= other.y2 && self.y2 >= other.y1 }
