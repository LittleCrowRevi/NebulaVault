...
use crate::systems::maps::{LEAF_DEV, new_map_test};
use crate::systems::maps::map_builder::{EventGrowBSPTree, Leaf};
use crate::systems::movement::movement::{left_walk_system, player_input};
...
fn setup_dev(mut commands: Commands) {
	...
	// Terminal
	let mut terminal = Terminal::new([DEVMAP_SIZE[0], DEVMAP_SIZE[1]])
	...
	// Camera
	commands.spawn(TiledCameraBundle::pixel_cam(VIEWPORT_SIZE)
		.with_pixels_per_tile([1, 1])
		.with_clear_color(Color::rgb(0.1, 0.1, 0.1))
	);
	
	// Map
	let map = new_map_test();
	commands.spawn((
		Map (map)
	));
...
impl Plugin for NebulaVault {
	fn build(&self, app: &mut App) {
		app
			...
			.add_systems(Update, (left_walk_system, render_all, player_input))
