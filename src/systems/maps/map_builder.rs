...
impl Room {
    pub fn default_room() -> Self {
        Self { dim: Rect::from_corners(vec2(0., 0.), vec2(10., 10.)) }
    }
    pub fn rectange_room() -> Self {
        Self { dim: Rect::from_corners(vec2(0., 0.), vec2(10., 20.)) }
    }

    pub fn thiny_room() -> Self {
        Self { dim: Rect::from_corners(vec2(0., 0.), vec2(5.0, 10.0)) }
    }

    pub fn thinx_room() -> Self {
        Self { dim: Rect::from_corners(vec2(0., 0.), vec2(10., 5.)) }
    }
}
...
pub fn split_leaf(container: &Leaf, depth: i16, min: (i32, i32), rng: [u8; 32]) -> Tree {
    let mut tree = Tree { leaf: *container, lchild: None, rchild: None };
...
pub fn generate_room(container: &Leaf, rng: [u8; 32], commands: &mut Commands, color: Color) {
    ...
    let max_w = container.w - 1;
    let max_h = container.h - 1;

    let rand_w = ChaCha20Rng::from_seed(rng).gen_range(1..=max_w.max(1));
    let rand_h = ChaCha20Rng::from_seed(rng).gen_range(1..=max_h.max(1));

    commands.spawn((
        SpriteBundle {
            transform: Transform::from_translation(vec3(container.x as f32, container.y as f32, 3.)),
            visibility: Visibility::Visible,
            sprite: Sprite { color, custom_size: Some(vec2(rand_w as f32, rand_h as f32)), ..default() },
            ..default()
        },
        Room { dim: Rect::from_corners(Vec2::ZERO, vec2(rand_w as f32, rand_h as f32)) },
    ));
}
...
pub fn random_split(container: &Leaf, min: (i32, i32), rng: [u8; 32]) -> (Leaf, Leaf) {
    ...
    if random() {
        ...
        left = Leaf { x: container.x - container.w / 2 + lw / 2, y: container.y, w: lw, h: container.h };
        right = Leaf { x: container.x + container.w / 2 - ((container.w - left.w) / 2), y: container.y, w: container.w - left.w, h: container.h };
    } else {
        let max_height = container.h - min.1;
        let lh = thread_rng().gen_range(min.1..=max_height.max(min.1));
        left = Leaf { x: container.x, y: container.y - container.h / 2 + lh / 2, w: container.w, h: lh };
        right = Leaf { x: container.x, y: container.y + container.h / 2 - ((container.h - left.h) / 2), w: container.w, h: container.h - left.h };
    }
    ...
    fn paint_box(self, commands: &mut Commands, color: Color, z_index: f32, border: f32) {
        ...
        commands.spawn((
            SpriteBundle {
                ...
                visibility: Visibility::Visible,
                sprite: Sprite { color, custom_size: Some(vec2(w - border, h - border)), ..default() },
                ..default()
            },
            self,
        ));
